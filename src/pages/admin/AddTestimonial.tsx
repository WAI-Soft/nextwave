...
import { testimonialService, CreateTestimonialData } from '../../services/testimonialService';
...
const AddTestimonial: React.FC<AddTestimonialProps> = ({ onSuccess, onCancel }) => {
  ...
  const handleInputChange = (field: keyof CreateTestimonialData, value: string | number | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Testimonial added successfully! It will appear on the home page.');
      onSuccess();
    } catch (error) {
  ...
  return (
    <Card className="bg-pure-black/40 border-champagne-gold/20" dir="ltr">
      <CardHeader>
        <CardTitle className="text-champagne-gold">Add New Testimonial</CardTitle>
        <CardDescription className="text-pure-white/70">
          Create a new client testimonial for the home page
        </CardDescription>
      </CardHeader>
      <CardContent>
